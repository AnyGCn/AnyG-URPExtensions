...
namespace UnityEngine.Rendering.Universal.Extensions.BakedShadowMap
{
    ...
    [ExecuteInEditMode]
    public class BakedShadowManager : MonoBehaviour
    {
        // Singleton Functions
        static BakedShadowManager s_Instance = null;

        public static BakedShadowManager instance
        {
            get
            {
                if (!s_Instance)
                {
                    GameObject go = new GameObject("Default " + typeof(BakedShadowManager).Name) { hideFlags = HideFlags.HideAndDontSave };
                    ...
                    s_Instance = go.AddComponent<BakedShadowManager>();
                }
